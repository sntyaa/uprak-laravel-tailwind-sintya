...
<div class="max-w-[720px] mx-auto py-32 sm:py-8 lg:py-16">
  ...
  <div class="w-full flex justify-between items-center mb-3 mt-1 pl-3">
    <div>
        <h3 class="text-lg font-semibold text-slate-800">Data Suppliers</h3>
        <p class="text-slate-500">A list of all the supplier data..</p>
    </div>
    <div class="ml-3">
        <div class="w-full max-w-sm min-w-[200px] relative">
            <div class="relative">
                <input
                  ...
                  placeholder="Search for supplier data..."
                />
                <button
    ...
    <table class="w-full text-left table-auto min-w-max">
      <thead>
        <tr>
          ...
          <th class="p-4 border-b border-slate-200 bg-slate-50">
            <p class="text-sm font-normal leading-none text-slate-500">Address</p>
          </th>
          <th class="p-4 border-b border-slate-200 bg-slate-50">
            <p class="text-sm font-normal leading-none text-slate-500">Phone Number</p>
          </th>
          <th class="p-4 border-b border-slate-200 bg-slate-50">
      ...
      <tbody>
        @foreach ($Suppliers as $Supplier)
          <tr class="hover:bg-slate-50 border-b border-slate-200">
            ...
            <td class="p-4 py-5">
              <p class="text-sm text-slate-500">{{ $Supplier->id }}</p>
            </td>
            <td class="p-4 py-5">
              <p class="text-sm text-slate-500">{{ $Supplier->id_supplier }}</p>
            </td>
            <td class="p-4 py-5">
              <p class="text-sm text-slate-500">{{ $Supplier->nama_supplier }}</p>
            </td>
            <td class="p-4 py-5">
              <p class="text-sm text-slate-500">{{ $Supplier->alamat_supplier }}</p>
            </td>
            <td class="p-4 py-5">
              <p class="text-sm text-slate-500">{{ $Supplier->telepon_supplier }}</p>
            </td>
            <td class="p-4 py-5">
              <a href="{{ route('Supplier.edit', $Supplier->id) }}" class="font-medium text-blue-600 hover:text-blue-800">Edit</a>
            </td>
            <td class="p-4 py-5">
