...
<div class="max-w-[720px] mx-auto py-32 sm:py-8 lg:py-16">
  ...
  <div class="w-full flex justify-between items-center mb-3 mt-1 pl-3">
    <div>
        <h3 class="text-lg font-semibold text-slate-800">Outgoing Items Data</h3>
        <p class="text-slate-500">A list of all the outgoing items data..</p>
    </div>
    <div class="ml-3">
        <div class="w-full max-w-sm min-w-[200px] relative">
            <div class="relative">
                <input
                  ...
                  placeholder="Search for outgoing items data..."
                />
                <button
    ...
    <table class="w-full text-left table-auto min-w-max">
      <thead>
        <tr>
          ...
          <th class="p-4 border-b border-slate-200 bg-slate-50">
            <p class="text-sm font-normal leading-none text-slate-500">Outgoing Date</p>
          </th>
          <th class="p-4 border-b border-slate-200 bg-slate-50">
            <p class="text-sm font-normal leading-none text-slate-500">Outgoing Amount</p>
          </th>
          <th class="p-4 border-b border-slate-200 bg-slate-50">
            <p class="text-sm font-normal leading-none text-slate-500">Location</p>
          </th>
          <th class="p-4 border-b border-slate-200 bg-slate-50">
            <p class="text-sm font-normal leading-none text-slate-500">Recipient</p>
          </th>
          <th class="p-4 border-b border-slate-200 bg-slate-50">
      ...
      <tbody>
        @foreach ($BarangKeluars as $BarangKeluar)
          <tr class="hover:bg-slate-50 border-b border-slate-200">
            ...
            <td class="p-4 py-5">
              <p class="text-sm text-slate-500">{{ $BarangKeluar->id }}</p>
            </td>
            <td class="p-4 py-5">
              <p class="text-sm text-slate-500">{{ $BarangKeluar->id_barang}}</p>
            </td>
            <td class="p-4 py-5">
            ...
            <td class="p-4 py-5">
              <a href="{{ route('BarangKeluar.edit', $BarangKeluar->id) }}" class="font-medium text-blue-600 hover:text-blue-800">Edit</a>
            </td>
            <td class="p-4 py-5">
              <a href="{{ route('BarangKeluar.destroy', $BarangKeluar->id) }}"
                 class="font-medium text-blue-600 hover:text-blue-800"
                 onclick="event.preventDefault(); if(confirm('Are you sure you want to delete this BarangKeluar')) { document.getElementById('delete-form-{{ $BarangKeluar->id }}').submit(); }">
                  Hapus
              </a>
